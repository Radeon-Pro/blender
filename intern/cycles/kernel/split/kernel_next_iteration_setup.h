                                            ...
                                            ccl_local_param unsigned int *local_queue_atomics)
{
  ...
  if (IS_STATE(ray_state_buffer, ray_index, RAY_HAS_ONLY_VOLUME)) {
    ASSIGN_RAY_STATE(ray_state_buffer, ray_index, RAY_ACTIVE);
  }
#endif

  bool active = IS_STATE(ray_state_buffer, ray_index, RAY_ACTIVE);
  if (active) {
    ccl_global float3 *throughput = &kernel_split_state_buffer(throughput, float3)[ray_index];
    ccl_global Ray *ray = &kernel_split_state_buffer(ray, Ray)[ray_index];
    ShaderData *sd = kernel_split_sd(sd, ray_index);
...
#ifdef __BRANCHED_PATH__
    if (!kernel_data.integrator.branched || IS_FLAG(ray_state_buffer, ray_index, RAY_BRANCHED_INDIRECT)) {
#endif
...
#  endif
  ...
  enqueue_ray_index_local(ray_index,
                          ...
                          IS_STATE(ray_state_buffer, ray_index, RAY_UPDATE_BUFFER) && active,
                          kernel_split_params.queue_size,
                          local_queue_atomics,
                          kernel_split_state_buffer(queue_data, int),
  ...
  if (IS_STATE(ray_state_buffer, ray_index, RAY_LIGHT_INDIRECT_NEXT_ITER)) {
...
#  ifdef __VOLUME__
  ...
  enqueue_ray_index_local(
      ...
      IS_STATE(ray_state_buffer, ray_index, RAY_VOLUME_INDIRECT_NEXT_ITER),
      kernel_split_params.queue_size,
      local_queue_atomics,
      kernel_split_state_buffer(queue_data, int),
      kernel_split_params.queue_index);
...
#  ifdef __SUBSURFACE__
  ...
  enqueue_ray_index_local(
      ...
      IS_STATE(ray_state_buffer, ray_index, RAY_SUBSURFACE_INDIRECT_NEXT_ITER),
      kernel_split_params.queue_size,
      local_queue_atomics,
      kernel_split_state_buffer(queue_data, int),
      kernel_split_params.queue_index);
